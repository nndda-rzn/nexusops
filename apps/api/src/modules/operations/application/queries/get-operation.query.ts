...
export async function getOperationQuery(
  ...
  const op = await findOperationByIdOrFail(query.operationId, query.orgId, db)
  return {
    id: op.id, type: op.type, status: op.status, priority: op.priority,
    referenceId: op.referenceId, referenceType: op.referenceType,
    isCrossEntity: op.isCrossEntity, relatedEntityIds: op.relatedEntityIds,
    ...
    cancelledBy: op.cancelledBy, cancellationReason: op.cancellationReason,
    createdBy: op.createdBy, createdAt: op.createdAt, updatedAt: op.updatedAt,
  }
}
